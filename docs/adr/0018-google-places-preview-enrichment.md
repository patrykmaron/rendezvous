...
- **In-process cache**, module-scope `Map`, keyed on the WHOLE query — `<fsq-id or "->" | <name> | <lat.toFixed(4)> | <lng.toFixed(4)>` — capped at 500 entries with FIFO eviction. Binding the key to name+coords (not to `fsq` alone) is deliberate: `fsq` is client-supplied and shared across rooms, so a bare `fsq:<id>` key let anyone poison a public Foursquare id with a wrong-venue result that every room then saw. With the full-query key, a poisoner must supply the true name+coords, at which point the haversine mismatch guard already bounds the result to the right venue. `fsq` is additionally validated against `^[A-Za-z0-9_-]{1,64}$` and ignored otherwise. Both positive and `not_found` results cache for 24h; a transport failure (`unavailable`) caches for only 5 minutes, so a Google outage degrades gracefully without poisoning every venue for a full day.
- **Billed-proxy defenses.** The route is an authenticated proxy to a metered Google SKU, so beyond the cache it also (a) coalesces identical concurrent misses onto a single in-flight Google call (N simultaneous clicks on a fresh winner bill once), and (b) rate-limits Google-call *attempts* on fixed 60s windows — 10/participant and 30 global per minute, keyed on the `requireMember` participant id, over-cap returning an uncached `unavailable` (200). Cache hits and coalesced awaiters never reach the limiter, so ordinary repeat clicks stay free.
- **`fsqPlaceId` threaded end-to-end** so the cache can key off it: `Venue.fsqPlaceId` (already fetched from ClickHouse) now survives `assemblePlanResult`/`publishFinalOverlay` in `room-agent.ts` into `PlanCandidate.venues[].fsqPlaceId` and `MapOverlay.pins[].placeId`, both optional so older persisted plan snapshots (predating this field) still parse. The room agent's `show_map` tool schema is unchanged — it stays strict (`additionalProperties: false`) with no `placeId` field, so mid-run model-painted pins simply have none; the `fsq`-less (`-` prefix) cache key covers them.
